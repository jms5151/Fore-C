...
gbr$Region <- "GBR"
gbr$Island <- gbr$Marine.Park.Mgmt.Section # for case-control grouping
gbr$Project <- "RHIS"

# add info to Guam data
guamRaw <- guamRaw %>% left_join(guam.gps, by="SITE")
...
guamRaw <- guamRaw %>% left_join(guamCodes, by="SPECIES")
guamRaw$Genus <- gsub("([A-Za-z]+).*", "\\1", guamRaw$Species)
guamRaw$Family <- ifelse((guamRaw$Genus %in% acroporids$Acroporidae$childtaxa_name) == TRUE, "Acroporidae", NA)
guamRaw$Family <- ifelse((guamRaw$Genus %in% poritids$Poritidae$childtaxa_name) == TRUE, "Poritidae", guamRaw$Family)
