...
T_gr <- seq(11, 36, 1)
GR <- rnorm(length(T_gr), (0.007 * T_gr * (T_gr - 10) * sqrt(37 - T_gr)), 1) #* 24
GR[GR<0] <- 0
N_gr <- length(T_gr)
# plot(T_gr, GR, pch=16)
...
T_r <- seq(11, 35, 1)
T_r <- seq(13, 32, 2)
Recovery <- rnorm(length(T_r), (T_r * -0.3 + 0.1), 1) 
...
# generated temperatures
sst <- read.csv("sst_tls.csv", head=T, stringsAsFactors=F)
...
init_fn <- function() {
  list(constant_gr=0.00005, T0_gr=8, Tm_gr=45, sigma_gr=0.03,
       constant_ip=0.00005, T0_ip=8, Tm_ip=45, sigma_ip=0.03)
}
...
# plot
plot(T_gr, GrowthRateMeans, type='l', lwd=2, ylab='Growth rate (generations/hr)', xlab=expression(paste("Temperature (",degree,"C)")), ylim=c(0,1.5))
...
# plot
plot(T_ip, InfectProbMeans, type='l', lwd=2, ylab='Infection probability', xlab=expression(paste("Temperature (",degree,"C)")), ylim=c(0,1.2))
...
# plot
plot(T_r, RecoveryMeans, type='l', lwd=2, ylab='Infectious period (days)', xlab=expression(paste("Temperature (",degree,"C)")), yaxt='n')
lines(T_r, RecoveryQuantiles[,2], lty=2, col='red')
lines(T_r, RecoveryQuantiles[,4], lty=2, col='red')
axis(side=2, at=-9:-3, labels=1:7)
